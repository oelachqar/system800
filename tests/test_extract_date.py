...
class MyTest(unittest.TestCase):
    ...
    def test_spellings_of_AMPM(self):
        self.assertDictEqual(date_info.extract_date_time('april thirteen two thousand nineteen at four thirty pm'),
            {'year': 2019, 'month': 4, 'day': 13, 'hour': 16, 'minute': 30})
        self.assertDictEqual(date_info.extract_date_time('april thirteen two thousand nineteen at four thirty p.m.'),
            {'year': 2019, 'month': 4, 'day': 13, 'hour': 16, 'minute': 30})
    ...
    def test_returnsNoneWhenTranscriptionFailed(self):
        self.assertDictEqual(date_info.extract_date_time('on march 2021 at 4:30 pm'), 
